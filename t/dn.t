...
(
	...
	[2, q|foo=1+bar=2|, q|foo=1+bar=2, baz=3|],
	[3, q|uid=jsmith|, q|UID=jsmith,DC=example,DC=net|],
	[3, q|ou=Sales+cn=J. Smith|, q|OU=Sales+CN=J. Smith,DC=example,DC=net|],
...
my($first);
my($get_count, $get_rdn, $get_type, $get_value);
my($result, $rdn_count);
my($text, $type);
my($value);
...
for my $item (@text)
{
	...
	if ($result == 0)
	{
		$get_count = $parser -> get_rdn_count;
		$get_rdn   = $parser -> get_rdn(1);
		$get_type  = $parser -> get_rdn_type(1)  || '';
		$get_value = $parser -> get_rdn_value(1) || '';

		ok($rdn_count == $get_count,  'get_rdn_count() works');   $count++;
		ok($first     eq $get_rdn,    'get_rdn($n) works');       $count++;
		ok($type      eq $get_type,   'get_rdn_type($n) works');  $count++;
		ok($value     eq $get_value,  'get_rdn_value($n) works'); $count++;
	}
}
...
for my $rdn (@rdn)
{
	...
	ok($value eq ${$parser -> get_rdn_values($type)}[0], "get_rdn_value($type) works"); $count++;
}
...
ok($rdn[0] eq 'example', "get_rdn_values('DC') works"); $count++;
ok($rdn[1] eq 'com',     "get_rdn_values('DC') works"); $count++;
